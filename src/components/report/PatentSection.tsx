...
export function PatentSection({ reportId, secrecy, responsible, contractor, contractors }: {
  ...
  const { value: file, setValue: setFile } = useReportBlock<PatentFile | null>(
    "patent", reportId, null, `geo_patent_${reportId}`,
  );
  ...
  const persist = (f: PatentFile | null) => setFile(f);
  ...
  const upload = async (raw: File) => {
    ...
    reader.onload = async () => {
      ...
      const res = await fetch(UPLOAD_URL, {
        ...
        body: JSON.stringify({ file: b64, filename: raw.name, contentType: "application/pdf", folder: `geo-patent-${reportId}` }),
      });
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          <Icon name="Award" size={18} className="text-geo-amber" />
          <h3 className="font-display text-xl tracking-wider uppercase text-foreground">Заключение о патентных исследованиях</h3>
        </div>
      ...
      <div className="border border-geo-amber/30 bg-geo-amber/5 px-4 py-3 flex items-center gap-3">
        ...
        <span className="font-mono text-xs text-geo-amber/80">Прикрепите заключение патентоведа в формате PDF</span>
      </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button onClick={() => openPreview(file.url, file.filename)}
                className="inline-flex items-center gap-1.5 text-xs font-mono text-geo-amber hover:text-geo-amber-hover transition-colors mt-2">
