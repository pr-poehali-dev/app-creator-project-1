  ...
  const upload = async (raw: File) => {
    ...
    reader.onload = async () => {
      ...
      const res = await fetch(UPLOAD_URL, {
        ...
        body: JSON.stringify({ file: b64, filename: raw.name, contentType: "application/pdf", folder: `geo-protocol-${reportId}` }),
      });
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h3 className="font-display text-xl tracking-wider uppercase text-foreground">Протокол рассмотрения отчёта</h3>
        </div>
      ...
      <div className="border border-geo-amber/30 bg-geo-amber/5 px-4 py-3 flex items-center gap-3">
        ...
        <span className="font-mono text-xs text-geo-amber/80">Прикрепите протокол рассмотрения (принятия) отчёта в формате PDF</span>
      </div>
