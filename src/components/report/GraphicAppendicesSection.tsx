...
export function GraphicAppendicesSection({ reportId, secrecy, responsible, contractor, contractors }: {
  ...
  const openAdd = () => {
    setForm({ title: "", scale: "", fileUrl: "", filename: "", uploadedAt: "" });
    setError(null);
    setModal("add");
  };

  const openEdit = (item: GraphicAppendix) => {
    setForm({ title: item.title, scale: item.scale, fileUrl: item.fileUrl || "", filename: item.filename || "", uploadedAt: item.uploadedAt || "" });
    setError(null);
    setModal(item);
  };
  ...
  const uploadFile = async (file: File) => {
    ...
    reader.onload = async () => {
      ...
      const res = await fetch(UPLOAD_URL, {
        ...
        body: JSON.stringify({ file: b64, filename: file.name, contentType: "application/pdf", folder: "geo-graphic-appendices" }),
      });
  ...
  const saveItem = () => {
    ...
    let next: GraphicAppendix[];
    if (modal === "add") {
      next = [...items, {
        id: Date.now().toString(), number: nextNumber,
        title: form.title.trim(), scale: form.scale,
        ...(form.fileUrl ? { fileUrl: form.fileUrl, filename: form.filename, uploadedAt: form.uploadedAt } : {}),
      }];
    } else {
      next = items.map((item) =>
        item.id === (modal as GraphicAppendix).id
          ? { ...item, title: form.title.trim(), scale: form.scale,
              ...(form.fileUrl
    ...
    <div className="animate-fade-in space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h3 className="font-display text-xl tracking-wider uppercase text-foreground">Список графических приложений</h3>
        </div>
      ...
      <div className="border border-geo-amber/30 bg-geo-amber/5 px-4 py-3 flex items-center gap-3">
        ...
        <span className="font-mono text-xs text-geo-amber/80">Нумерация автоматическая · масштаб обязателен для картографических материалов · файл PDF необязателен</span>
      </div>
      ...
      {items.length === 0 ? (
        <div className="border border-dashed border-border py-16 text-center">
          <Icon name="Map" size={28} className="text-muted-foreground/20 mx-auto mb-3" />
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    {item.scale ? (
                      <span className="font-mono text-xs text-geo-amber border border-geo-amber/30 px-1.5 py-0.5">{item.scale}</span>
                    ) : (
          ...
          <div className="bg-card border border-border w-full max-w-lg p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h4 className="font-display text-sm tracking-wider uppercase">
                {modal === "add" ? `Граф. приложение № ${nextNumber}` : `Граф. приложение № ${(modal as GraphicAppendix).number}`}
              </h4>
            ...
            <div className="space-y-2">
              <label className="text-xs font-mono text-muted-foreground uppercase tracking-widest">
                Масштаб
                <span className="ml-2 normal-case text-muted-foreground/40 font-mono">для нечисловых материалов — «без масштаба»</span>
              </label>
              <div className="flex gap-2 flex-wrap">
                {COMMON_SCALES.map((s) => (
                  <button key={s} type="button" onClick={() => setForm((f) => ({ ...f, scale: s }))}
                    className={`px-2.5 py-1 text-xs font-mono border transition-colors ${form.scale === s ? "bg-geo-amber text-primary-foreground border-geo-amber" : "bg-muted text-muted-foreground border-border hover:border-geo-amber/50 hover:text-foreground"}`}>
                    {s}
                  </button>
                ))}
                <button type="button" onClick={() => setForm((f) => ({ ...f, scale: "без масштаба" }))}
                  className={`px-2.5 py-1 text-xs font-mono border transition-colors ${form.scale === "без масштаба" ? "bg-geo-amber text-primary-foreground border-geo-amber" : "bg-muted text-muted-foreground border-border hover:border-geo-amber/50 hover:text-foreground"}`}>
                  без масштаба
                </button>
              </div>
              <input type="text" value={form.scale} onChange={(e) => setForm((f) => ({ ...f, scale: e.target.value }))}
